...
pub struct ScratchBuffers {
    ...
    /// Dequantized weight scratch for BLAS prefill. Sized to the largest weight
    /// matrix used in prefill (max of `3*hs*hs`, `is*hs`). Reused across all
    /// GEMM call sites within a single forward pass and across calls.
    pub dequant_weight: Vec<f32>,
}
    ...
    pub fn from_config_with_compression(config: &ModelConfig, compression: &KvCompression) -> Self {
        ...
        Self {
            ...
            scratch: ScratchBuffers {
                ...
                // Largest weight matrix in prefill is max(3*hs*hs, is*hs, hs*is).
                // Allocated up front so the BLAS prefill path never reallocates.
                dequant_weight: vec![
                    0.0;
                    (3 * config.hidden_size * config.hidden_size)
                        .max(config.intermediate_size * config.hidden_size)
                ],
            },
