...
fn run_parity(model_name: &str, tokens: &[u32]) -> Option<(f32, f32, usize, usize)> {
    ...
    let model_a = wick::model::load_model(gguf_a, None, 8192).unwrap();
    ...
    let model_b = wick::model::load_model(gguf_b, None, 8192).unwrap();
...
fn test_flash_vs_naive_prefill_parity() {
    ...
    let model_c = wick::model::load_model(gguf_c, None, 8192).unwrap();
    ...
    let model_d = wick::model::load_model(gguf_d, None, 8192).unwrap();
